...
def detect_changes(original_df, edited_df):
    ...
    for term in deleted_terms:
        ...
        changes.append({
            'type': 'delete',
            'term': term,
            'old': {'term': term, 'meaning': original_row['meaning'], 'status': original_row['status']},
            'new': None
        })
...
def save_to_db():
    ...
    changes = detect_changes(st.session_state.original_df, st.session_state.current_edited_df)
    ...
    st.session_state.local_terms = dataframe_to_terms(st.session_state.current_edited_df)
...
def filter_dataframe(df, status_filters, search_query):
    ...
    if search_query:
        ...
        meaning_mask = filtered_df['meaning'].str.contains(search_lower, case=False, na=False)
...
def bulk_approve_filtered():
    ...
    filtered_df = filter_dataframe(st.session_state.current_edited_df, selected_statuses, search_term)
    count = 0
...
def bulk_delete_filtered():
    ...
    for _, row in filtered_df.iterrows():
        ...
        old_value = st.session_state.current_edited_df[
...
with top_col1:
    if st.button(t("update_db"), type="primary", disabled=num_changes == 0, use_container_width=True):
        saved_count = save_to_db()
        st.rerun()
with top_col2:
    if st.button(t("undo"), disabled=len(st.session_state.change_history) == 0, use_container_width=True):
        undo_last_change()
        st.rerun()
with top_col3:
...
if st.session_state.current_edited_df is not None:
    ...
    total, pending, approved, disapproved = get_stats(st.session_state.current_edited_df)
            ...
            with st.container():
                ...
                with col2:
                    ...
                    if new_meaning != term_row['meaning']:
                        mask = st.session_state.current_edited_df['term'] == term_row['term']
                        old_meaning = st.session_state.current_edited_df.loc[mask, 'meaning'].iloc[0]
                        st.session_state.current_edited_df.loc[mask, 'meaning'] = new_meaning
                        record_change('meaning', term_row['term'], old_meaning, new_meaning)
                ...
                with col3:
                    ...
                    if new_status != current_status:
                        mask = st.session_state.current_edited_df['term'] == term_row['term']
                        st.session_state.current_edited_df.loc[mask, 'status'] = new_status
                        record_change('status', term_row['term'], current_status, new_status)
                
                with col4:
                    if st.button(t("delete"), key=f"delete_{term_row['term']}_{i}"):
                        # Store old value for potential undo
                        old_value = st.session_state.current_edited_df[
                            st.session_state.current_edited_df['term'] == term_row['term']
                        ].iloc[0].to_dict()
