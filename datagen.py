...
def extract_terms(text, min_length=3):
    ...
    valid_tokens = [t for t in tokens if t.isalpha() and len(t) >= min_length]
...
def generate_test_groups(all_terms, group_size=1000):
    ...
    group3.extend(
        all_terms[
            group_size * 2 : group_size * 2 + (group_size - g1_sample - g2_sample)
        ]
    )
...
def create_term_objects(terms):
    ...
    return [{"term": term, "meaning": f"Definition of {term}"} for term in terms]
...
print(
    f"Group 3: {len(g3)} terms ({len(s1 & s3)} overlap with G1, {len(s2 & s3)} with G2)"
)
