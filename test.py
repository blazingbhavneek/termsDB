...
def simulate_human_approval(manager, approval_rate=0.7, disapproval_rate=0.2):
    ...
    # Approve some
    for term in random.sample(pending, min(approved, len(pending))):
        manager.update_status(term["term"], "approved")
    
    # Disapprove some
    remaining = [t for t in pending if t not in pending[:approved]]
    for term in random.sample(remaining, min(disapproved, len(remaining))):
        manager.update_status(term["term"], "disapproved")
    
    print(f"\nSimulated approval: {approved} approved, {disapproved} disapproved")
...
def main():
    ...
    for r in results:
        print(f"\n{r['group']}:")
        print(f"  Time: {r['time_ms']:.2f}ms | Filtered: {r['filtered']} terms")
