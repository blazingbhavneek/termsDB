...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allows all origins. In production, specify your React app's URL(s) e.g., ["http://localhost:3000"]
...
@app.get("/terms")
async def get_terms(statuses: str = Query(None, description="Comma-separated list of statuses to filter by (e.g., 'pending,approved')")):
    try:
        status_list = None
        if statuses:
        ...
        for change in changes:
            ...
            try:
                if change_type == 'status':
                    ...
                    results.append({"error": f"Unknown change type '{change_type}' for term '{term_name}'"})
            except ValueError as ve:
                 results.append({"error": f"Term '{term_name}' not found: {str(ve)}"})
            except Exception as e:
                 results.append({"error": f"Failed to apply change for '{term_name}': {str(e)}"})
